...
class SingleAgentEnv:
    ...
    def _resample_commands(self, envs_idx):
        target_tensor = torch.stack(self.target_locations)      # convert target_locations to pre-stacked tensor
        new_indices = (self.current_target_index[envs_idx] + 1) % len(self.target_locations)
        self.commands[envs_idx] = target_tensor[new_indices]
        self.current_target_index[envs_idx] = new_indices
    ...
    def reset_idx(self, envs_idx):
        ...
        # reset drone position to random location near a target
        # first generate a random target index
        rand_target_idx = random.randint(0, len(self.target_locations) - 1)
        # now spawn the drones just before the target location associated with this index
        self.base_init_pos = (self.target_locations[rand_target_idx] - 1.0 * self.gate_tangents[rand_target_idx])
        ...
        self.current_target_index[envs_idx] = -1 + rand_target_idx
        self._resample_commands(envs_idx)
