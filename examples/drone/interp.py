...
def spline_interpolant(x_data, y_data,
    ...
    return scipy_spline_to_casadi(spline, extrapolate, name = name)

def scipy_spline_to_casadi(spline, extrapolate = 'const', name = 'k'):
    '''
    ...
    x_fit = ca.SX.sym('s')
    # make x_fit periodic arounf k_x[0] and k_x[-1]
    # x_fit1 = ca.if_else(x_fit < k_x[0], x_fit + k_x[-1] - k_x[0], x_fit)
    # x_fit2 = ca.if_else(x_fit1 > k_x[-1], x_fit1 - k_x[-1] + k_x[0], x_fit1)
    x_fit1 = ca.fmod(x_fit - k_x[0], k_x[-1] - k_x[0])
    x_fit2 = ca.if_else(x_fit1 < k_x[0], x_fit1 + k_x[-1] - k_x[0], x_fit1)

    x_0 = ca.pw_const(x_fit2, k_x, [k_x[0], *k_x])
    c_0 = ca.pw_const(x_fit2, k_x, [k_c[3,0], *k_c[3,:], k_f])
    if extrapolate == 'const':
        c_1 = ca.pw_const(x_fit2, k_x, [0,        *k_c[2,:], 0])
    elif extrapolate == 'linear':
        c_1 = ca.pw_const(x_fit2, k_x, [k_c[2,0], *k_c[2,:], spline(k_x[-1],1)])
    else:
        raise NotImplementedError(f'Unrecognized extrapolation key: {extrapolate}')


    c_2 = ca.pw_const(x_fit2, k_x, [0,        *k_c[1,:], 0])
    c_3 = ca.pw_const(x_fit2, k_x, [0,        *k_c[0,:], 0])

    x_rel = x_fit2 - x_0
    y_fit = c_0 + c_1*x_rel + c_2*x_rel**2 + c_3*x_rel**3
    f_y = ca.Function(name, [x_fit], [y_fit])
    # f_fit1 = ca.Function('fit_1', [x_fit], [x_fit1])
