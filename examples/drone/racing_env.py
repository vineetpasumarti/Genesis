...
class HoverEnv:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False, device="cuda"):
        ...
        # self.target_locations = [
        #     torch.tensor([-1.1, -1.6, 3.6], device=self.device),
        #     torch.tensor([9.2, 6.6, 1.0], device=self.device),
        #     torch.tensor([9.2, -4, 1.2], device=self.device),
        #     torch.tensor([-4.5, -6, 3.5], device=self.device),
        #     torch.tensor([-4.5, -6, 0.8], device=self.device),
        #     torch.tensor([4.75, -0.9, 1.2], device=self.device),
        #     torch.tensor([-2.8, 6.8, 1.2], device=self.device),
        #     torch.tensor([1.1, -1.6, 3.6], device=self.device)
        # ]
        self.current_target_index = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)

    def _resample_commands(self, envs_idx):
        for idx in envs_idx:
            target_index = self.current_target_index[idx]
            self.commands[idx] = self.target_locations[target_index]
            self.current_target_index[idx] = (target_index + 1) % len(self.target_locations)
    ...
    def step(self, actions):
        ...
        self.reset_buf = (self.episode_length_buf > self.max_episode_length) | self.crash_condition | all_targets_reached
    ...
    def reset_idx(self, envs_idx):
        ...
        self.current_target_index[envs_idx] = 0
        self._resample_commands(envs_idx)
